#!/usr/bin/env python3
"""Run the JD detail extractor against sanitized local HTML fixtures."""
...
def extract(fixture: str, sku_id: str = "100000000001") -> dict:
    browser("open", (FIXTURES / fixture).as_uri())
    browser("wait", "--load", "domcontentloaded")
    js = subprocess.run([sys.executable, str(ROOT / "scripts" / "extract.py"), sku_id], text=True, capture_output=True, check=True).stdout
...
def main() -> None:
    try:
        ...
        assert detail["id"] == "100000000001"
        assert detail["price"]["amount"] == 199.0
        assert detail["price"]["originalAmount"] == 229.0
        assert detail["skuResolved"] is False
        assert detail["skuGroups"][0]["options"][0]["label"] == "黑色"
        assert detail["attributes"]["品牌"] == "脱敏品牌"
        assert detail["shippingText"] == "配送至：脱敏地区"
        ...
        print("jd-product-detail contract: ok")
    finally:
