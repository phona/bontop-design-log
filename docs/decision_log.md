...
- **决策人**：业主
- **后续修订**：形态参数（height 2.1→2.55 到顶、panels 3→4 两固两活、叠收方式）由 DEC-2026-08-26-044 修订；本条的元素类型/交互/碰撞机制不变
...
### DEC-2026-08-26-044 餐厨推拉门改版：到顶极窄边长虹玻璃 + 四扇两固两活
...
- **选定方案**：
  ...
  - **四扇两固两活**（panels 3→4）：西固 0.9 + 中间两活 1.8m 对开 + 东固 0.9；东固正对冰箱南侧板（fridge @(10.45,2.05)，南脸恰与门线 z=2.4 共面），该段为死段不损通行；open 状态中间两扇就近叠收至西/东固定扇（东端叠收与冰箱并排，参考图同款构图）
  - **材质**：极窄边哑光黑铝框（16~25mm，落"黑色载体"规则）+ 超白长虹竖纹玻璃（与卫浴黑窄边长虹门、DEC-043 主卫屏风同语言）；吊轨无地轨（保 DEC-040 地砖通铺不断）
  - 渲染：`renderSlidingDoorRun` open 布局改为首尾固定+中间就近叠收两端（原全叠收西端）
