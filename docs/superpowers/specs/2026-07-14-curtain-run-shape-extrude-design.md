...
- 圆心位于角平分线外侧；
- 用 `Shape.absarc(center.x, center.z, r, startAngle, endAngle)` 绘制中心线圆弧。
...
中心线圆弧确定后，需把整条中心线沿法向两侧各偏移 `T/2`（`T = GLASS_THICKNESS`），形成闭合的 ribbon 边界：
- 对直线段做等距平行偏移；
- 对圆弧段做同心圆弧偏移：凸侧半径 `r + T/2`，凹侧半径 `max(0, r - T/2)`；若 `r ≤ T/2`，凹侧退化为尖角；
- 偏移后拼接成连续外边界 `O` 与内边界 `I`；
- 最终 `Shape` 沿 `O → （开放时封口） → I → （开放时封口） → O` 闭合。

> 偏移曲线实现是本次变更的核心几何算法。首次实现先覆盖 **闭合多边形**（当前玻璃幕墙）和 **简单开放路径**（L 形）；复杂自交路径不在本次范围。
