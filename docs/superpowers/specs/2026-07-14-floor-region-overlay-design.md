...
`bay_sill` 的 `points` 必须落在当前 `overlay.yaml` 的 `curtain_run` 上，不能按房间矩形随意定义。当前 `curtain_run` 有效外墙段为：

- 西墙：x=-5.88, z=-2.99 → 4.872（长 7.862m）
- 北墙：z=5.39, x=-5.363 → 3.745（长 9.108m）
- SW 对角线：(-0.578, -4.323) → (-5.75, -3.17) → (-5.88, -2.99)（长 5.521m）

为避免主卧与次卧飘窗重叠，按房间在 curtain_run 上的投影重新划分：主卧占西墙、北墙左段、SW 对角线；两个次卧分别占西墙下段和北墙右段。

### 初版 bay_sill 清单（修正后）

| id | points | 长度 | 说明 |
|----|--------|------|------|
| `master_left_bay` | (-5.88, -0.93) → (-5.88, 4.39) | 5.32m | 主卧西墙 |
| `bedroom_nw_bay` | (-5.88, -2.99) → (-5.88, -0.93) | 2.06m | 西墙下段，次卧 |
| `master_top_bay` | (-5.363, 5.39) → (-2.23, 5.39) | 3.133m | 主卧北墙 |
| `bedroom_se_bay` | (-2.23, 5.39) → (0.39, 5.39) | 2.62m | 北墙右段，次卧 |
| `master_bottom_bay` | (-0.578, -4.323) → (-5.75, -3.17) | 5.299m | 主卧 SW 对角线 |

> 注：
> 1. 所有坐标取自当前 `curtain_run` 外墙线，确保不超出幕墙范围。
> 2. `width` 由 `points` 线段长度自动算出，配置里不声明。
> 3. 量房或新数据到达后可单独调整某段 `points`。当前坐标基于 `cad-extracted.yaml` 房间矩形与 `curtain_run` 的投影，若实际户型边界有出入可再校准。
